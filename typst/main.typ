...
= Related Works / Current Literature
...
Over the years, numerous studies have been conducted in the domain of electroencephalography (EEG), making it an important area of active research in cognitive neuroscience. The Brain Imaging Data Structure @gorgolewski_brain_2016 orginally proposed for magnetic resonance imaging (MRI) is a community standard for data organization and sharing of brain data within research communities. This standard is designed using the FAIR (findability, accessibility, interoperability, and reusability) principles @wilkinson_fair_2016 contributing to efficient scientific data management and stewardship. Following the release of BIDS standard a lot of extensions concerning different medical imaging and neuroimaging techniques like magnetoencephalography (MEG) @niso_meg-bids_2018 , intracranial electroencephalography (iEEG) @holdgraf_ieeg-bids_2019, electroencephalography (EEG) @pernet_eeg-bids_2019, Positron Emission Tomography (PET) @norgaard_pet-bids_2022, etc are developed. Data sharing and reuse of datasets  was eventually simplified using open source sharing platforms like OpenNeuro @markiewicz_openneuro_2021 which hosts more than 600 datasets in compliance with BIDS. In similar line, @dobson_presentations_2018 also contributed to an automatic generation of shareable, version-controlled BIDS datasets from MR scanners. 

Concering the conversion to BIDS, packages like `mne-bids` @appelhoff_mne-bids_2019 and BIDSCoin @zwiers_bidscoin_2022 helps to convert our raw EEG data to BIDS compliant datasets and finally validates the conversion with the bids validator @gorgolewski_brain_2016. To facilitate the analysis of these converted and validated BIDS data in cross platform setups and multi tenant clusters @gorgolewski_bids_2017 developed a cross platform containerized framework to ease the usage of BIDS data analysis pipelines.@zwiers_bidscoin_2022 also published a graphical user interface (GUI) to lower the barrier of BIDS adaptation among researchers. To add to this ease of data management, @covitz_curation_2022 adds another layer where users can validate, summarize BIDS metadata, etc along with version control using Datalad @halchenko_datalad_2021. Version control is an important aspect in data collection to ensure data traceability and an error-free workflow. Studies with large number of participants like the Narratives collection @nastase_narratives_2021, `highspeed` datasets @wittkuhn_dynamics_2021, used Datalad for versioning and organizing their datasets. Such data management workflows, incorporating all of the above aspects except version control i.e. from raw data acquisition to organization/standardization to analysis, are dealt with Stawiski et al., @stawiski_optimizing_2024. In this study, they exhibit a prototype of collecting heterogenous raw data with more than 100 participants for a Deep Brain Simulation Research (DBS) including BIDS compliant checks and database management using SQLite and finally using Python based libraries like Numpy for the analysis pipeline.

In our study, we attempt to incorporate all the aspects for an efficient data management pipeline and automate the entire process of archiving our recorded data in real time in a standardized BIDS format and depositing it in a Dataverse repository via version control using DataLad.
