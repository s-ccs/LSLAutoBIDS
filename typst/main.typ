...
*Data Management:* To allow machines and humans to navigate such a diverse dataset, the Brain Imaging Data Structure (BIDS) @gorgolewski_brain_2016 standard provides a well-described framework, offering consistent directory structure, file naming conventions, and metadata descriptors for neuroimaging data. The recorded dataset can then be seamlessly integrated into analysis pipelines, promoting data sharing.
...
All the above works are efforts by different research groups towards achieving effective Research Data Management (RDM) @noauthor_rdm_nodate, which involves managing and organizing research data like deciding where to store it, who has access it, how long to keep it, how it’s shared, and the guiding rules throughout the research lifecycle. The RDM Handbook @noauthor_rdm_nodate acts as a standardized reference for managing such pipelines of data collection, processing, and archiving in a project.
...
In addition to LSL-based EEG data acquisition, `LSLAutoBIDS` supports the incorporation of non-LSL data. For example, in our use-case, eye-tracking data are collected using the EyeLink 1000 Plus eye tracker simultaneously with EEG data collection, which produces proprietary Eyelink data format (EDF) files which cannot easily be streamed via LSL. Other examples are an electronic lab notebook record, log-files of the experiment, and a compressed archive of the experimental code used for any each subject individually. `LSLAutoBIDS` accommodates these files as a secondary data modality currently implemented via a netshare drive, and is able to organize them within the appropriate BIDS subdirectories to publish them alongside EEG data.
...
After each recording session, the dataset is then uploaded to a Dataverse repository, where it is persistently published with the specified metadata. After data collection is finished, the dataset can there be versioned for release, and publicly shared with an appropriate data sharing agreement.
...
The LSLAutoBIDS package is continuously developed and freely available: #link("https://github.com/s-ccs/LSLAutoBIDS") or via Zenodo(2025) #link("10.5281/zenodo.15525821").
